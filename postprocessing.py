...
parser.add_argument('--weight_path', default = None, help = 'the path to the corrected weight, by default the matrix is set to a single-valued matrix.', type = str)
parser.add_argument('--camera_transformation_path', default = os.path.join('configs', 'T_camera1_camera2.npy'), help = 'the path to the transformation matrix between cameras', type = str)
FLAGS = parser.parse_args()
begin_id = int(FLAGS.begin_id)
end_id = int(FLAGS.end_id)
corrected = FLAGS.corrected
if corrected:
    corrector = PoseCorrector(object_file_name_list = FLAGS.object_file_name_list, perspective_num = FLAGS.perspective_num, perspective_pair_weight_path = FLAGS.weight_path)
...
for id in tqdm(range(begin_id, end_id + 1)):
    ...
    renderer.render_scene(os.path.join(FLAGS.data_dir, 'scene{}'.format(id)), use_corrected_pose = corrected)
    renderer_L515.render_scene(os.path.join(FLAGS.data_dir, 'scene{}'.format(id)), use_corrected_pose = corrected)
